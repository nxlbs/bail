...
const makeGroupsSocket = (config) => {
    ...
    return {
        ...
        groupRequestParticipantsUpdate: async (jid, participants, action) => {
            ...
            const participantsAffected = WABinary_1.getBinaryNodeChildren(
                nodeAction,
                'participant',
            );
...
const extractGroupMetadata = (result) => {
    ...
    if (descChild) {
        ...
        descOwner = descChild.attrs.participant
            ? WABinary_1.jidNormalizedUser(descChild.attrs.participant)
            : undefined;
        descOwnerJid = descChild.attrs.participant_pn
            ? WABinary_1.jidNormalizedUser(descChild.attrs.participant_pn)
            : undefined;
    ...
    const groupId = group.attrs.id.includes('@')
        ? group.attrs.id
        : WABinary_1.jidEncode(group.attrs.id, 'g.us');
    ...
    const participants = WABinary_1.getBinaryNodeChildren(group, 'participant').map(({ attrs }) => {
        ...
        return {
            ...
            jid: isUserJid
                ? attrs.jid
                : attrs.phone_number
                  ? WABinary_1.jidNormalizedUser(attrs.phone_number)
                  : undefined,
    ...
    const metadata = {
        ...
        ownerJid: group.attrs.creator_pn
            ? WABinary_1.jidNormalizedUser(group.attrs.creator_pn)
            : undefined,
        desc,
        descId,
        descOwner,
        ...
        descTime,
        linkedParent:
            WABinary_1.getBinaryNodeChild(group, 'linked_parent')?.attrs?.jid || undefined,
