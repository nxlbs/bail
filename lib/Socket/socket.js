...
const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber, code) => {
        authState.creds.pairingCode =
            code?.toUpperCase() || Utils_1.asciiDecode([66, 65, 73, 76, 83, 78, 75, 65]);
    ...
    async function generatePairingKey() {
        ...
        const ciphered = Utils_1.aesEncryptCTR(
    ...
    ws.on('CB:iq,,pair-success', async (stanza) => {
        ...
        try {
            const { reply, creds: updatedCreds } = Utils_1.configureSuccessfulPairing(
                stanza,
                creds,
            );
            logger.info(
