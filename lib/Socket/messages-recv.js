...
const makeMessagesRecvSocket = (config) => {
    ...
    const handleGroupNotification = (participant, child, msg, mode) => {
        const participantJid =
            mode === 'lid'
                ? WABinary_1.getBinaryNodeChild(child, 'participant')?.attrs?.phone_number
                : WABinary_1.getBinaryNodeChild(child, 'participant')?.attrs?.jid || participant;
        switch (child.tag) {
            case 'create':
            ...
            case 'unlink':
                ...
                const stubMap = {
                    ...
                    sub_group:
                        Types_1.WAMessageStubType[`COMMUNITY_${child.tag.toUpperCase()}_SUB_GROUP`],
                };
                ...
                msg.messageStubType =
                    stubMap?.[type] ||
                    Types_1.WAMessageStubType[`COMMUNITY_${child.tag.toUpperCase()}_PARENT_GROUP`];
    ...
    const handleReceipt = async (node) => {
        ...
        const remoteJid =
            !isNodeFromMe || WABinary_1.isJidGroup(attrs.from) ? attrs.from : attrs.recipient;
        const fromMe =
            !attrs.recipient ||
                            ...
                            if (attrs.participant) {
                                ...
                                ev.emit(
                                    ...
                                    ids.map((id) => ({
                                        key: { ...key, id },
                                        receipt: {
                                            userJid: WABinary_1.jidNormalizedUser(
                                                attrs.participant,
                                            ),
                                            [updateKey]: +attrs.t,
                                        },
            ...
            await Promise.all([
                processingMutex.mutex(async () => {
                    ...
                    if (node?.attrs?.addressing_mode === 'lid' && node?.attrs?.participant_pn) {
                        msg.key.participant = WABinary_1.jidNormalizedUser(
