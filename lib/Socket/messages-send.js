...
const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (
        ...
        await authState.keys.transaction(async () => {
            ...
            if (
                Utils_1.normalizeMessageContent(message)?.pinInChatMessage ||
                Utils_1.normalizeMessageContent(message)?.keepInChatMessage
            ) {
...
*/
    ...
    return {
                                ...
                                if (
                                    ...
                                    throw new boom_1.Boom(
                                        `Media re-upload failed by device (${resultStr})`,
                                        {
                                            data: media,
                                            statusCode:
                                                Utils_1.getStatusCodeForMediaRetry(media.result) ||
                                                404,
                                        },
                                    );
                                }
        ...
        sendStatusMentions: async (jid, content) => {
            ...
            let statusJid = Private
                ? [jid]
                : (await groupMetadata(jid)).participants.map((num) => num.id);
            ...
            let msg = await Utils_1.generateWAMessageFromContent(
                jid,
                {
                    [type]: {
                        message: {
        ...
        sendAlbumMessage: async (jid, medias, options = {}) => {
            ...
            const album = await Utils_1.generateWAMessageFromContent(
                jid,
                {
                    albumMessage: {
