...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('interactiveButtons' in message && !!message.interactiveButtons) {
        let mediaMessage = {}, mediaAttachment = false;
        if ('image' in message) {
            mediaMessage = message?.image ? await (0, exports.prepareWAMessageMedia)({ image: message.image, ...options }, options) : null;
            mediaAttachment = true
        }
        const video = message?.header?.video ? await (0, exports.prepareWAMessageMedia)({ video: message.header.video, ...options }, options) : null;
        const document = message?.header?.document ? await (0, exports.prepareWAMessageMedia)({ document: message.header.document, ...options }, options) : null;
