...
const prepareWAMessageMedia = async (message, options) => {
    ...
    const fileEncSha256B64 = (
        options.newsletter
            ? fileSha256
            : fileEncSha256 !== null && fileEncSha256
              ? fileEncSha256
              : fileSha256
    ).toString('base64');
            ...
            try {
                if (requiresThumbnailComputation) {
                    const { thumbnail, originalImageDimensions } =
                        await messages_media_1.generateThumbnail(
                            originalFilePath,
                            mediaType,
                            options,
                        );
                ...
                if (requiresWaveformProcessing) {
                    uploadData.waveform = await messages_media_1.getAudioWaveform(
                        originalFilePath,
                        logger,
                    );
...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('text' in message) {
        ...
        if (contactLen === 1) {
            contactMessage = {
                contactMessage: Types_1.WAProto.Message.ContactMessage.fromObject(
                    message.contacts.contacts[0],
                ),
            };
        } else {
            contactMessage = {
                contactsArrayMessage: Types_1.WAProto.Message.ContactsArrayMessage.fromObject(
                    message.contacts,
                ),
            };
        }
        ...
        if (message.live) {
            locationMessage = {
                liveLocationMessage: Types_1.WAProto.Message.LiveLocationMessage.fromObject(
                    message.location,
                ),
            };
        } else {
            locationMessage = {
                locationMessage: Types_1.WAProto.Message.LocationMessage.fromObject(
                    message.location,
                ),
            };
        }
            ...
            case 'interactive':
                m.interactiveResponseMessage = {
                    body: {
                        ...
                        format: WAProto_1.proto.Message.InteractiveResponseMessage.Body.Format
                            .EXTENSIONS_1,
                    },
    ...
    if ('sections' in message && !!message.sections) {
        ...
        const thumbnail = message.thumbnail
            ? await messages_media_1.generateThumbnail(message.thumbnail, 'image')
            : null;
        ...
        const interactiveMessage = {
            nativeFlowMessage: {
                ...
                ...(message.paramsJson ? { messageParamsJson: message.paramsJson } : {})
            },
        };
...
const updateMessageWithReaction = (msg, reaction) => {
    ...
    const reactions = (msg.reactions || []).filter(
        (r) => generics_1.getKeyAuthor(r.key) !== authorID,
    );
...
const updateMessageWithPollUpdate = (msg, update) => {
    ...
    const votes = (msg.pollUpdates || []).filter(
        (r) => generics_1.getKeyAuthor(r.pollUpdateMessageKey) !== authorID,
    );
    ...
    for (const update of pollUpdates || []) {
        ...
        for (const option of vote.selectedOptions || []) {
            ...
            voteHashMap[hash].voters.push(
                generics_1.getKeyAuthor(update.pollUpdateMessageKey, meId),
            );
        }
    }
...
const downloadMediaMessage = async (message, type, options, ctx) => {
    ...
    async function downloadMsg() {
        ...
        const stream = await messages_media_1.downloadContentFromMessage(
            download,
            mediaType,
            options,
        );
...
async function prepareStickerPackMessage(stickerPack, options) {
    const { stickers, name, publisher, packId, description, cover } = stickerPack;
    ...
    try {
        ...
        if (cover) {
            ...
            trayBuffer = await messages_media_1.toBuffer(
                (await messages_media_1.getStream(stickers[0]?.data)).stream,
            );
        }
        ...
        stickerData[`${stickerPackIdValue}.png`] = [new Uint8Array(thumbnailBuffer), { level: 0 }];

        const thumbUpload = await messages_media_1.encryptedStream(thumbnailBuffer, 'image', {
    ...
    const stickerPromises = stickers.map(async (s, i) => {
        ...
        const hash = crypto_2.sha256(buffer).toString('base64').replace(/\//g, '-');
    ...
    const zipBuffer = await new Promise((resolve, reject) => {
        fflate_1.zip(stickerData, (err, data) => {
            if (err) {
                reject(err);
            } else {
    ...
    const stickerPackMessage = {
        ...
        stickerPackOrigin:
            WAProto_1.proto.Message.StickerPackMessage.StickerPackOrigin.USER_CREATED,
