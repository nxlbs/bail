...
const cleanMessage = (message, meId) => {
    ...
    message.key.participant = message.key.participant
        ? WABinary_1.jidNormalizedUser(message.key.participant)
        : undefined;
...
const getChatId = ({ remoteJid, participant, fromMe }) => {
    if (
        WABinary_1.isJidBroadcast(remoteJid) &&
...
const processMessage = async (
    ...
    if (protocolMsg) {
        switch (protocolMsg.type) {
            case WAProto_1.proto.Message.ProtocolMessage.Type.HISTORY_SYNC_NOTIFICATION:
                ...
                if (process) {
                    ...
                    const data = await history_1.downloadAndProcessHistorySyncNotification(
                        histNotification,
                        options,
                    );
        ...
        const participantsIncludesMe = () =>
            participants.find((jid) => WABinary_1.areJidsSameUser(meId, jid));
        switch (message.messageStubType) {
            ...
            case Types_1.WAMessageStubType.GROUP_PARTICIPANT_INVITE:
                ...
                if (
                    ...
                    !participants.includes(message?.key?.participant)
                ) {
