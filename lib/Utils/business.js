...
const parseCatalogNode = (node) => {
    ...
    return {
        products,
        nextPageCursor: paging ? WABinary_1.getBinaryNodeChildString(paging, 'after') : undefined,
    };
};
const parseCollectionsNode = (node) => {
    const collectionsNode = WABinary_1.getBinaryNodeChild(node, 'collections');
    const collections = WABinary_1.getBinaryNodeChildren(collectionsNode, 'collection').map(
        (collectionNode) => {
            ...
            const products = WABinary_1.getBinaryNodeChildren(collectionNode, 'product').map(
                parseProductNode,
            );
            return {
                id,
